...
import { ULKELER, ULKE_SLUG, bayrakEmoji, ulkeIstasyonSluglari } from "@/lib/ulkeler";

// Ülke sayfası paylaşım kartı — bayrak + başlık + istasyon sayısı.
export const size = { width: 1200, height: 630 };
export const contentType = "image/png";
export const alt = "Ülke radyoları — ŞİMDİ";

export default async function OG({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const kod = ULKE_SLUG[slug];
  const u = kod ? ULKELER[kod] : null;
  const baslik = !u ? "Dünya Atlası" : kod === "www" ? "İnternet Radyoları" : `${u.tr} Radyoları`;
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 28, color: "#8f8f96" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {adet > 0 ? `${adet} istasyon · canlı · şu an ne çalıyor` : "canlı · şu an ne çalıyor"}
          </div>
          <div style={{ display: "flex", color: "#fafafa" }}>dinle →</div>
        </div>
      </div>
    ),
    { ...size, emoji: "twemoji" }
